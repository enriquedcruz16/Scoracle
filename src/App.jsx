...
function StandTab({allFix,live,predictions}){
  ...
  function liveGroupTable(gKey){
    ...
    allFix.filter(function(f){return(f.group||"").toUpperCase().replace(/GROUP\s*/i,"").trim()===gKey&&(f.isDone||f.isLive)&&!f.isKnockout;}).forEach(function(fix){
  ...
  return(<div style={{padding:16}}>
    ...
    {view==="liveworld"&&<>
      {!isKnockoutPhase&&<>
        ...
        {allFix.some(function(f){return f.isLive&&(f.group||"").toUpperCase().replace(/GROUP\s*/i,"").trim()===g;})&&(
          <div style={{display:"inline-flex",alignItems:"center",gap:4,background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:20,padding:"3px 10px",fontSize:10,fontWeight:700,color:"#ef4444",marginBottom:12}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:"#ef4444"}}/>Live match in progress
          </div>
        )}
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{liveRows.map(function(r,i){return(
              <tr key={r.team} style={{borderTop:"1px solid #0f0f0f",borderLeft:i<2?"3px solid #22c55e":i===2?"3px solid #f59e0b":"none"}}>
                ...
                <td style={{padding:"12px 8px",fontSize:13}}><span style={{marginRight:6}}>{FLAGS[r.team]||"🏳️"}</span><span style={{fontWeight:600}}>{r.team}</span></td>
        ...
        {allFix.filter(function(f){return(f.group||"").toUpperCase().replace(/GROUP\s*/i,"").trim()===g&&!f.isKnockout;}).map(function(fix){
          const result=live[fix.id]||(fix.isDone?{homeGoals:fix.homeGoals,awayGoals:fix.awayGoals}:null);
      ...
      {isKnockoutPhase&&<>
        <div style={{display:"flex",gap:6,overflow:"auto",paddingBottom:4,marginBottom:14,scrollbarWidth:"none"}}>
          {[{id:"r32",label:"R32"},{id:"r16",label:"R16"},{id:"qf",label:"QF"},{id:"sf",label:"SF"},{id:"final",label:"Final"}].map(function(r){return(
            <button key={r.id} onClick={function(){setRound(r.id);}} style={{background:round===r.id?"#f59e0b":"#0f0f0f",border:round===r.id?"none":"1px solid #1f1f1f",color:round===r.id?"#000":"#6b7280",borderRadius:20,padding:"6px 16px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,outline:"none"}}>{r.label}</button>
          );})}
        </div>
...
function StatsTab({allFix,predictions,live,totalPts,predCount,totalFix,bonus,allBonusAnswers,currentUser}){
  ...
  return(<div style={{padding:16}}>
    ...
    <div style={{background:"#080808",border:"1px solid #141414",borderRadius:14,padding:14,marginBottom:8}}>
      {[{label:"Tournament Winner",pts:champPts,pending:!champResult,pick:get("champion"),result:champResult,max:PTS_WINNER},{label:"Golden Boot",pts:bootPts,pending:!bootResult,pick:get("topscorer"),result:bootResult,max:PTS_BONUS},{label:"Most Group Goals",pts:goalsPts,pending:!goalsResult,pick:get("mostgoals"),result:goalsResult,max:PTS_BONUS}].map(function(row){return(
